...
window.onRightClick = function(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (hit.uv) {
            let terrainType = 'Terre'; // PAR DEFAUT: C'est la terre (Sécurité si tu joues en local)
            
            // On essaie de lire l'image. Si le navigateur bloque, le try catch empêche le plantage !
            try {
                ...
                console.warn("Lecture du Radar bloquée par le navigateur local. Clic forcé sur Terre !");
            }
...
function expandTerritory() {
    ...
    let gainedTerritory = Math.floor(growth); playerStats.territory += gainedTerritory; playerStats.maxPop += (gainedTerritory * 20); 
    updateHUD();
}

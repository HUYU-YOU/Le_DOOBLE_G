...
// --- 1. GESTION DES PARAMETRES (Blindée pour window) ---
...
window.stopSettingsAnim = function() { 
    ...
    const btn = document.getElementById('settings-btn-img'); 
    if (btn && !btn.src.includes('settings4.png')) { btn.src = '../img/setting.png'; } 
}
...
window.clickSettingsAnim = function() { 
    ...
    setTimeout(() => { if(btn) btn.src = '../img/setting.png'; }, 300); 
}
...
window.setGameSize = function(size) { 
    ...
    let btnClassic = document.getElementById('btn-sz-classic'); let btnWide = document.getElementById('btn-sz-wide'); 
...
function launchMissile(siloEntity, targetPos) {
    ...
    function animateMissile() {
        if (index < points.length) {
            ...
            window.gameScene.remove(missile); createExplosion(end);
        }
    }
    animateMissile();
}
